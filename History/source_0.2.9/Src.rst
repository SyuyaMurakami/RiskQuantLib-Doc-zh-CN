...
有了Src文件夹提供的组件功能，我们可以很好地解决这个问题。RiskQuantLib将数据结构和数据处理逻辑分离，我们所有的处理逻辑都被放进了Src文件夹，而数据结构则由用户自行定义，工程结构则由Build_Attr.xlsx和Build_Instrument.xlsx来保存。当我们需要由回测转向生产时，我们仅仅需要更换Src文件夹下面的内容，同时保持其他部分不变。
